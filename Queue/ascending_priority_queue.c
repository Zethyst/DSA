...
void ENQUEUE(struct Node **front, int d, int p)
{
    ...
    { // Traverse the list and find a position to insert new node by comparing new node's priority with the old ones
      //Jaise hi usko koi badi priority milegi loop tutega aur ptr ki value save hogi

        while (ptr->next != NULL && ptr->next->Priority <= New->Priority)
        {
            ptr = ptr->next;
        }
        New->next = ptr->next; // Could be either the end of the list or at required position
        ptr->next = New;       //make new links
    }
}
...
void SEARCH(struct Node *front, int value)
{
    ...
    if (flag == 1)
        printf("\nThe Element was found successfully on position number %d\n", count);
    else
